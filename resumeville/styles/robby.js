...
function moveUpLayerHorizontal() {
    if ("horizontal" == layersMovement) {
        ...
        clearInterval(shiftUpLayerHorizontalTimer), positionHorizontalLayersAtBottomMost(), positionHorizontalLayersToHaveSameRightPosition(), isRobbyBelowSeaLevel = !1
}
...
function moveDownLayerHorizontal() {
    if ("horizontal" == layersMovement) {
        ...
        clearInterval(shiftDownLayerHorizontalTimer), positionHorizontalLayersAtBottomMost(), positionHorizontalLayersToHaveSameRightPosition(), isRobbyBelowSeaLevel = !1
}
...
function positionLayerHorizontalToTop() {
    if (1 == isRobbySwimming) {
        ...
        for (var e = 0; e < layerHorizontalArray.length; e++)
            layerHorizontalArray[e].style.top = -shiftUpLayerHorizontalDistance + "px";
        for (e = 0; e < layerVerticalArray.length; e++)
            layerVerticalArray[e].style.bottom = shiftUpLayerHorizontalDistance + "px"
    }
}
...
function checkRobbyJumpFallSwim() {
    if ("horizontal" == layersMovement)
        if (1 == isRobbySwimming)
            1 == isRobbyBelowSeaLevel && robbySwimUp();
        else
...
function positionVerticalLayersHorizontally() {
    for (var e = 0; e < layerVerticalArray.length; e++)
        layerVerticalArray[e].style.left = layerHorizontalArray[e].offsetLeft + layerHorizontalArray[e].offsetWidth - containerDiv.offsetWidth + "px"
}
...
function animatePlants() {
    for (var e = 0; e < plantArray.length; e++)
        $(plantArray[e]).stop().delay(300 * e).animate({
            top: [plantTargetTopObjectArray[e].offsetTop, "easeOutElastic"]
        }, 800, function() {})
}
function positionPlants() {
    for (var e = 0; e < plantArray.length; e++)
        plantArray[e].style.top = 1 == canAnimatePlantInformation ? "100%" : plantTargetTopObjectArray[e].offsetTop + "px"
}
function animateBuildings() {
    ...
    for (var e = 0; e < buildingArray.length; e++)
        $(buildingArray[e]).stop().delay(300 * e).animate({
...
function animateBuildingsLegs() {
    buildingCounter += 1;
    for (var e = 0; e < buildingArray.length; e++) {
        if (buildingArray[buildingArray.length - 1].offsetLeft == buildingTargetLeftArray[buildingTargetLeftArray.length - 1])
            return buildingLegFrameArray[buildingLegFrameArray.length - 1].style.left = "0px", clearInterval(buildingLegsTimer), void (buildingCounter = 0);
...
function blinkBuildings() {
    if (pageVerticalPosition + .5 * containerDiv.offsetWidth < about2ContainerDiv.offsetLeft || pageVerticalPosition + .5 * containerDiv.offsetWidth > about2ContainerDiv.offsetLeft + about2ContainerDiv.offsetWidth)
...
function nbaBoardsJump() {
    for (var e = 0; e < nbaBoardArray.length; e++)
        $(nbaBoardArray[e]).stop().delay(100 * e).animate({
...
function shiftRobbyFrame() {
    ...
    if (1 == isRobbySwimming && 1 == isRobbyBelowSeaLevel ? (robbyStartFrame = robbyStartSwimFrame, robbyStopFrame = robbyStopSwimFrame) : (robbyStartFrame = robbyStartRunFrame, robbyStopFrame = robbyStopRunFrame), robbyFramesDiv.style.left = -1 * robbyOneFrameWidth * (robbyStartFrame + counter) + "px", robbyStopFrame < robbyStartFrame + counter + switcher && (switcher *= -1), robbyStartFrame + counter + switcher == robbyStartFrame && (pageVerticalPositionWhenAnimateRobby1 = pageVerticalPosition), robbyStartFrame + counter + switcher < robbyStartFrame) {
        if (pageVerticalPositionWhenAnimateRobby1 == (pageVerticalPositionWhenAnimateRobby2 = pageVerticalPosition))
            return clearShiftRobbyFrameTimer(), void ("not moving 2" == layersMovement && robbyHandsUp());
        switcher *= -1
    }
    counter += switcher
}
...
function openSquidHands() {
    for (var e = 0; e < squidHandOpenArray.length; e++)
        squidHandOpenArray[e].style.opacity = 1, squidHandOpenArray[e].style.filter = "alpha(opacity=100)";
    for (e = 0; e < squidHandCloseArray.length; e++)
        squidHandCloseArray[e].style.opacity = 0, squidHandCloseArray[e].style.filter = "alpha(opacity=0)"
}
function closeSquidHands() {
    for (var e = 0; e < squidHandOpenArray.length; e++)
        squidHandOpenArray[e].style.opacity = 0, squidHandOpenArray[e].style.filter = "alpha(opacity=0)";
    for (e = 0; e < squidHandCloseArray.length; e++)
        squidHandCloseArray[e].style.opacity = 1, squidHandCloseArray[e].style.filter = "alpha(opacity=100)"
}
...
function blinkSeaAnimals(e) {
    ...
    for (a = 0; a < i.length; a++)
        $(i[a]).fadeTo(0, 1), $(i[a]).stop().delay(300).animate({
            opacity: 0
        }, 0, function() {})
}
...
function positionContactConfirmationContainer() {
    for (var e = 0; e < contactConfirmationContainerArray.length; e++)
        contactConfirmationContainerArray[e].style.left = "not moving 1" == layersMovement || "not moving 2" == layersMovement ? robbyContainerDiv.offsetLeft + "px" : robbyMaxHorizontalDistance + "px", contactConfirmationContainerArray[e].style.top = .8 * containerDiv.offsetHeight - 370 + "px"
}
...
function drawOneLayerOfFirework() {
    if (fireworkLayerNumber < fireworkRowNumber) {
        ...
        fireworkLayerNumber = 0, clearInterval(drawOneLayerOfFireworkTimer), makeFireworkDisappear(drawFireworkCounter), drawFireworkCounter += 1
}
